...
clicks_updated %>%
    ...
    labs(x = "Hour", y = "Count",
         ...
         caption = "The figure represents average clicks per hour by weekday in our data set.") +
...
clicks_updated %>%
    ...
    labs(x = "Hour", y = "Count",
         title = "Average Clicks per Hour",
         caption = "The figure represents total clicks per hour in our data set. ") +
    scale_x_continuous(breaks = seq(0,23, length.out=24), minor_breaks = NULL) +
    scale_color_brewer(palette = "Set1", name = "Weekday") +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
...
click_order_join_upd %>% 
    ...
    group_by(created_at_date, paid, itinerary_with_baggage) %>% 
    summarise(Count = n(), .groups = "keep") %>% 
    ...
    labs(x = "Date", y = "Count", 
         title = "Bar Plot of Purchased Tickets Over Time", 
         subtitle = "Faceted on whether baggage was included", 
         caption = "The figure shows two plots: the FALSE plot shows orders that included baggage; the TRUE plot vice versa.")
...
click_order_join_upd %>% 
    ...
    labs(x = "Hour", y = "Count", 
         ...
         caption = "The figure shows two plots: the FALSE plot shows orders that were created through a mobile site; \nthe TRUE plot vice versa.")
```

```{r, fig.height=7, fig.width=3}
click_order_join_upd %>% 
    filter(paid %in% c(F,T),
           created_at_date < "2023-01-30") %>% 
    group_by(created_at_hour, created_at_wday, paid) %>%
    summarise(Count = n(), .groups = "keep") %>% 
    # plot start
    ggplot(aes(x = created_at_hour, y=Count, col=paid)) +
    geom_line(size=1) +
